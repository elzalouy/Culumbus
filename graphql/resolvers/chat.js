...
module.exports={
    ...
    deleteMessage:async (args) => {
        ...
        // if(args.isAuth){
            ...
            // if(user){
                try{
            await client.conversations.conversations(args.chat)
    ...
    startChat: args => {
        ...
        return User.findOne({mobileNumber:args.mobileNumber}).then(user=>{
            if(user && !user.startedChat){
                ...
                .participants
                .create({'identity': args.mobileNumber}).then(par => {
                    return client.conversations.conversations(conversation.sid)
                    .participants
                    .create({'identity': 'AdminCulumbus'}).then(par2 => {
    ...
    startChatSocial: args => {
        ...
        return User.findOne({socialID:args.socialID}).then(user=>{
            if(user && !user.startedChat){
                return client.conversations.conversations
                .create({
                    chatServiceSid: sid,
                ...
                return client.conversations.conversations(conversation.sid)
                .participants
                .create({'identity': user.chatID}).then(par => {
                    return client.conversations.conversations(conversation.sid)
                    .participants
                    .create({'identity': 'AdminCulumbus'}).then(par2 => {
                        user.chat=conversation.sid
